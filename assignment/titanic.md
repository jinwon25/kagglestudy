...
| **방법**     | **개념**                                                                 |
|--------------|------------------------------------------------------------------------|
| **Bagging**   | 데이터를 여러 번 샘플링해서 여러 모델 학습 → 결과를 평균/투표                        |
| **Boosting** | 이전 모델이 틀린 데이터를 다음 모델이 보완하면서 학습                            |
| **Stacking** | 여러 개의 서로 다른 모델들의 예측 결과를 모아서, 또 다른 모델(메타 모델)로 학습                           |
| **Voting**   | 여러 분류기의 예측을 다수결(Hard) 또는 확률 평균(Soft)으로 결정              |
